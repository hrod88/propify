...
import { mockGastosComunes, mockUnidades, mockUsers, formatCLP } from '@/lib/mock-data'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const gasto  = mockGastosComunes.find(g => g.id === id)
  if (!gasto) return { title: 'Gasto' }
  const unidad = mockUnidades.find(u => u.id === gasto.unidadId)
...
export default async function GastoDetailPage({ params }: PageProps) {
  ...
  const residente = uid ? mockUsers.find(u => u.id === uid) : undefined
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap mb-1">
              <h1 className="text-2xl font-bold text-gray-900">
                Unidad {unidad?.numero ?? gasto.unidadId}
              </h1>
              <span
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-5">
          ...
          {/* Info de pago (si está pagado o parcial) */}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-xl" style={{ background: '#f8fafc' }}>
                  ...
                  <p className="text-sm font-bold text-gray-900 mt-0.5">
                    {new Date(gasto.fechaPago).toLocaleDateString('es-CL', {
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-red-600 mt-0.5">
                  Este cobro no ha sido pagado después de la fecha de vencimiento.
                  Se recomienda enviar un recordatorio al residente.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="space-y-5">
            ...
            {unidad ? (
              <div className="space-y-0">
                ...
                <Link
                  ...
                  className="block text-xs font-semibold mt-3 hover:opacity-75 transition-opacity"
                  style={{ color: '#2563ae' }}
                >
                  Ver unidad →
                </Link>
          ...
          {gasto.estadoPago !== 'pagado' && (
            <div className="space-y-2">
              ...
              <button
                className="w-full py-2.5 rounded-xl text-sm font-semibold hover:opacity-80 transition-opacity"
